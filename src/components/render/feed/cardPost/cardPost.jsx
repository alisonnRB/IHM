      ...
      <div className='cardPost'>
        <span className='infosPost'>
          ...
          <span className='publiUser'>
            <img className='perfilPubli' src={autor && autor.fotoPerfil ? "http://literary-ihm.com/imagens/" + autor.fotoPerfil : ""} onError={(e) => { e.target.src = noF; }} />
          ...
          {link && link != undefined ? <Link to={`/Ler/?id=${encodeURIComponent(JSON.stringify(link.id))}`}>
            <div className='imgLinkBox'>
              ...
              {semIMG ? <div className="noIMAGE" >
                {link.nome}
              </div> : <img src={link.imagem ? "http://literary-ihm.com/livros/" + link.user_id + '/' + link.nome + '_' + link.id + '/' + link.imagem : ""} className='imgLink' />}
